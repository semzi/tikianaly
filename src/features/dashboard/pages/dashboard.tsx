      ...
      try {
        ...
        const upsertLeagueFixtures = (leagueId: number, leagueFixtures: any[]) => {
          setFixtures((prev) => {
            ...
            next.push({ leagueId, fixtures: leagueFixtures });
            next.sort(
              (a, b) =>
        ...
        const fetchLeague = async (leagueId: number) => {
          try {
            ...
            console.log(
              `Response for leagueId ${leagueId} and date ${formattedDate}:`,
              response
            ); // Added log
            if (
              response?.success &&
        ...
        <div className="w-full pb-30 flex flex-col gap-y-3 md:gap-y-5 lg:w-3/5 h-full overflow-y-auto hide-scrollbar pr-2">
          ...
          <div className="flex flex-col gap-y-3 md:gap-y-6">
            ...
            <div className="hidden md:block">
              {topLeagueIds.map((leagueId, leagueIdx) => {
                  ...
                  {leagueFixture.fixtures.map((game: any, gameIdx: number) => (
                    ...
                    >
                      {game.status === "FT" ? (
                        ...
                        <p className="text-xs text-brand-secondary animate-pulse text-center w-15 px-2 font-bold">{game.time}</p>
                      ) : (
                        <p className="text-xs dark:text-snow-200 text-neutral-n4 text-center w-15 px-2 font-medium">{game.time}</p>
                      )}
                      <div className="flex flex-col flex-1 mx-1 gap-0.5">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-1">
