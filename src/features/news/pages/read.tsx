...
export default function BlogPostClient({
	...
	useEffect(() => {
		...
		const load = async () => {
			...
			try {
				...
				if (mounted) {
					setPost(fetched);
					setComments(Array.isArray(fetched?.comments) ? fetched?.comments : []);
					setError(null);
				}
			} catch (err: unknown) {
				if (mounted) setError(err instanceof Error ? err.message : "Failed to load post");
			} finally {
	...
	const handleReact = async (type: ReactionType) => {
		...
		try {
			...
			const key = `post:${effectiveId}:reaction`;
			if (typeof window !== "undefined" && effectiveId) localStorage.setItem(key, type);
			setHasReacted(true);
			await refreshPost();
		} catch {
			setSelected(null);
	...
	if (!post) {
		return (
			<div className="flex items-center justify-center min-h-[60vh]">
				<p className="text-neutral-n4">{isRefreshing ? 'Loading post…' : 'Post not found'}</p>
			</div>
		);
	}
			...
			<div className="w-full md:w-7/10 lg:w-7/10">
				...
				<div className="mt-5 flex flex-col-reverse overflow-y-auto gap-3 text-sm h-90 edge-lighting pb-0 px-0 block-style ">
					...
					<div className="flex mx-5 h-full overflow-y-auto hide-scrollbar flex-col gap-3">
						<p className="font-[500] mb-1 flex items-center sz-4 theme-text">Comments</p>
						{comments.length === 0 ? (
